    ...
    <div className="w-full h-full flex flex-row items-stretch justify-center gap-8 py-8 px-8 min-h-0 max-h-full">
      ...
      <div className="flex-1 flex flex-col gap-4 min-h-0">
        <div className="flex flex-row items-center gap-3">
          ...
          <h1 className="text-xl font-black uppercase tracking-wider text-white">ENTRENADOR AI</h1>
        </div>
      ...
      <div className="w-[400px] flex flex-col rounded-3xl overflow-hidden shadow-2xl bg-white/10 backdrop-blur-md border border-white/10 min-h-0">
        <div className="p-6 border-b border-white/10 bg-white/5">
          <h2 className="text-2xl font-bold text-white tracking-wide">Transcripción en vivo</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages && messages.map((msg, index) => (
            ...
            >
              <span className={`text-xs mb-1 px-2 ${msg.sender === 'user' ? 'text-blue-300' : 'text-pink-300'}`}>
...
}> = ({ mode, sessionAccessToken, onSessionStopped, onSessionComplete, idInteraction }) => {
  return (
    <LiveAvatarContextProvider sessionAccessToken={sessionAccessToken} idInteraction={idInteraction} onSessionComplete={onSessionComplete}>
